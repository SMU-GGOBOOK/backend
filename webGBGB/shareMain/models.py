...
class ReadingGroup(models.Model):
    ...
    admin = models.ForeignKey(Member, on_delete=models.CASCADE,related_name='main_owned_groups')  # 세션아이디 로그인중인 유저로 방장 설정
    member = models.ManyToManyField(Member,related_name='main_reading_groups',blank=True)  # 방장 제외 나머지 멤버가 들어옴, null이어도 ok
    description = models.TextField(blank=True)  # 그룹소개글
    book = models.ForeignKey(Book, on_delete=models.CASCADE, default=1, related_name='main_owned_groups')  # book모델 연결
